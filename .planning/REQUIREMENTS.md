...
- [x] **NL-ACC-02**: NL→SPARQL predicate/schema-convention grounding — mechanically-derived TBox predicate index (label + domain + range + object-vs-datatype + usage-shape, walked purely from `rdfs:domain`/`rdfs:range` declarations, no hand-curated per-schema hints so it transfers to CDF, D-02) injected as a new engine seam-7 prompt block (`predicate_index()`/`predicate_prompt_section()` on `QueryLanguageAdapter`, D-06/D-07), composed after the entity block inside `NLQueryEngine._system_prompt`, targeting the 17 convention-bound CK25 failures left over after entity grounding. Prove a **statistically significant, execution-graded** CK25 lift over the NL-ACC-01 entity-grounded baseline (`baseline.json:openai-gpt4o-mini-ck25-grounded` = 14/49), McNemar p<0.05, zero regressions; directional QALD-9-plus generalization check (first-ever live run of the powered gate, zero-shot + predicate-grounded — reported, not gated, D-03). No CK25-specific hand-curation (mechanical schema walk only, D-02); scripted configs stay the sole CI-reachable default; W3C DAWG coverage ≥96.4% and the SPARQL→AQL transpiler package unchanged (D-08). **CLOSED via the DOCUMENTED-NULL path (07.4-05, credentialed live sweep 2026-07-27, NL-FEW-02 precedent — NOT a passed confirmatory lift):** standalone seam-7 (predicate-alone, entity block removed) REGRESSES vs a fresh same-session entity-alone arm (5/49 vs 12/49, paired McNemar b=0/c=7/p=0.0156 — significant in the WRONG direction, 7 losses/0 gains); the phase's actually-intended composition, seam-7 ADDITIVE on top of seam-6 entity grounding, is a pure statistical wash vs a fresh same-session entity-alone arm (13/49 vs 13/49, p=1.0, bootstrap paired_delta=0.0, CI₉₅[-0.0816, 0.0816], b=2/c=2 — 2 gains/2 losses cancel, and it is NOT zero-regression). QALD-9-plus directional generalization sits at the floor for both arms (1/514 zero-shot, 3/514 predicate-grounded; achieved MDE ≈5.5–6.2pt at n=514, so +2 cases is noise, never gated per D-03/D-04). The seam-7 prompt wording (mechanical TBox walk, D-02/D-06/D-07) is provisional; documented follow-up: expand render detail across all predicates (not only the 17 convention-bound CK25 failures originally targeted) or try an alternative retrieval/selection strategy for which predicates to surface. — acceptance: live execution-graded CK25 predicate-grounded config shows a significant positive delta (McNemar p<0.05) over the recorded entity-grounded baseline with zero regressions; QALD-9-plus zero-shot-vs-predicate-grounded directional delta recorded (not gated); scripted configs stay the no-network CI default; W3C DAWG coverage ≥96.4% and the SPARQL→AQL transpiler package unchanged — evidence: `tests/nl2sparql/eval/baseline.json:configs.openai-gpt4o-mini-ck25-predicate-grounded.confirmatory_test`, `:phase07_4_predicate_grounding_sweep.ck25_additive_arm`, `:phase07_4_predicate_grounding_sweep.qald9plus_directional`. **⚠️ PROVISIONAL/CONFOUNDED (07.4-06, code review CR-01):** the sweep above ran against a "dump mode" that never actually dumped the schema — widening `k` to the total predicate count was a no-op against the shared `arango_query_core` scorer's zero-hit filter, so a typical CK25 question saw only 1–8 of 30 predicates, not the full schema the D-01 design intended. 07.4-06 fixed this upstream (`arango_query_core` commit `b669320`, real `dump=True` kwarg) and re-pinned/re-wired both adapters, but did NOT re-run the credentialed live CK25 sweep (human checkpoint required, no key held by the agent). This documented-null disposition should be treated as unconfirmed until that re-run happens — do not cite it as a settled negative result.
