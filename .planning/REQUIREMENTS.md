...
- [x] **REQ-thirdparty-tool-compat** (PRD §3.10): Every §11.1 verified-compatible tool row has a passing smoke test (≥1 SELECT, 1 ASK, Service Description fetch) — Protégé, YASGUI, SPARQLWrapper, MS Ontology Playground. — acceptance: `tests/integration/test_*_compat.py`
- [x] **REQ-ontoextract-integration** (PRD §3.11): `arango-ontoextract` can point its Q7 endpoint at us, seed via `/mapping/export-owl`, accept a curated OWL push via `/mapping/import-owl`. — acceptance: `tests/integration/test_aoe_roundtrip.py` (Docker-gated)
- [x] **REQ-performance-slos** (PRD §3.12): Every §9.4 perf budget row passes within ≤ 25% of stated p95 (CI-blocking on > 25% regression). — acceptance: `tests/perf/test_*.py`
...
- [x] **NL-ACC-03**: NL→SPARQL relationship-path grounding — a mechanically-derived, **subclass-aware, inverse-edged** TBox class-connectivity graph (engine **seam-8** `ClassPathIndex` in `arango_query_core.nl`, stdlib-only, `pyoxigraph`-free, no per-schema hand-curation so it transfers to CDF; Cypher inherits) that, given a question's grounded **anchor class** (seam-6) and its **target** (seam-7 token scorer), retrieves the *specific* bounded shortest predicate path(s) connecting them (depth ≤3, ≤5-path global-pool budget, deterministic ordering: length → question-token overlap → lexical) and injects them as a compact **navigation-hint** block composed *after* the entity block — targeting the **16 verified "right-entity, wrong-path" CK25 failures** (valid SPARQL that executes to empty) left after entity+few-shot grounding (composed `g+f` ceiling = 16/49), presenting a shared-variable join/star pattern (D-04), NOT a schema dump (the 07.4 distraction that net-regressed `all` 12/49 < `g+f` 16/49). Reuses seams 6/7 unchanged. Graded **directionally + zero-regression** (CK25 n=49 underpowered, mirroring 07.3–07.5), gated first by an offline path-recall pre-gate. Root cause + design (D-1…D-10) + GREEN Step-0 spike: `.planning/research/relationship-path-grounding-scope.md`. **CLOSED via the DOCUMENTED-NULL path (07.6-03 Task 3, credentialed live 6-arm composed sweep 2026-08-10, promoted+pinned engine `1ec67899e762dac4deef904c7324e06c61ead15b`, NL-ACC-02/NL-FEW-02 precedent — NOT an adopted lift):** the offline R4 path-recall pre-gate held GREEN (14/16, matching SPEC.md's floor exactly) through the engine promotion, and Task 1's `--dry-run` proved all three seams (grounding + few-shot + path) build into one `NlPipeline` (scripted 49/49). The credentialed same-session 6-arm sweep (n=49, gpt-4o-mini, `judge=execution`, `corpus_sha=814d227`) reproduced every prior isolated-session finding cleanly in one run — `ground vs zero` b=7/c=0/p=0.0156, `fewshot vs zero` b=8/c=1/p=0.0391, `g+f vs zero` b=11/c=0/p=0.0010 (the strongest composition, 16/49) — but the phase's own contrast, `g+f+path vs g+f` (the additive seam-8 lift over the two already-confirmed levers), came back **b=0 gains / c=1 regression / p=1.0 / Δ=−0.0204, CI₉₅[−0.0612, 0.0000]**: the path-hint arm (15/49) scored *below* the g+f baseline (16/49) it was composed on, with zero offsetting gains and one distraction regression (`ck25-6`). This **fails the R5 adopt bar** (directional lift AND c=0). The substantive finding is the offline-recall-green / live-composition-null tension: `ClassPathIndex.shortest_paths` mechanically finds the correct predicate path for 14/16 empty-result golds, but that mechanical recoverability did not convert into an end-to-end accuracy lift once the navigation-hint was composed into an already-strong g+f prompt — mirroring NL-ACC-02's seam-7 documented-null (mechanically-sound predicate index, non-significant/negative once additively composed, p=0.6875) and NL-FEW-02's dense-few-shot documented-null (p=0.453). The seam-8 engineering artifact itself (subclass-aware D-9, inverse-edged D-2, bounded self-revisit D-10, deterministic ordering, no-`pyoxigraph`-import D-08) is real, unit-tested, and promoted to both remotes for reuse (Cypher inherits it) — only the accuracy lever failed to convert. `g+f` (16/49) remains the strongest composition in this codebase after 4 grounding-lever phases (07.3 entity / 07.4 predicate / 07.5 few-shot / 07.6 path); neither seam-7 nor seam-8 improves on it additively, both closing via documented-null. — acceptance: offline path-recall **≥14/16** on the empty-result golds (always-on tier, no key) [MET, 14/16]; engine unit tests green (subclass-aware D-9 + inverse D-2 + bounded self-revisit D-10 + deterministic ordering + no-`pyoxigraph`-import D-08) [MET]; `--dry-run` composed arm builds grounding+few-shot+path into one `NlPipeline`, scripted arm 49/49 [MET]; the human-run credentialed CK25 paired sweep (`grounded+few-shot+path` vs `grounded+few-shot`, same session) shows a directional lift with **zero regressions** ⇒ ADOPT recorded in `baseline.json` (or documented-null) [NOT MET — documented-null instead, b=0/c=1/p=1.0/Δ=-0.0204]; mechanical TBox derivation only (no CK25 hand-curation) [MET]; W3C DAWG coverage ≥96.4% and the SPARQL→AQL transpiler package unchanged [MET] — evidence: `tests/nl2sparql/eval/baseline.json:phase07_6_path_grounding_sweep`, `tests/nl2sparql/eval/test_path_recall.py`

- [x] **NL-GEN-01**: NL→SPARQL query-first synthetic few-shot bank — a **build-time** generator that, given any ontology's TBox (+ representative instance data), produces a per-ontology `(question, gold-SPARQL)` few-shot bank **query-first**: hand-written ontology-agnostic compositional SPARQL templates (lookup / value-object / category-filter / COUNT / top-N / OFFSET / negation / 2-hop) instantiated from the TBox (reusing the Phase 07.4 shape classification), data-bound + execution-filtered to non-empty, with paired question templates + K LLM paraphrases for faithful natural questions; loaded through the existing BM25 `FewShotIndex` seam (engine-side, Cypher inherits). Language-/schema-agnostic — the **same generator** runs on CK25 and QALD. Consciously re-opens Phase 07.1's synthetic-generator out-of-scope in the narrower query-first / few-shot-*pool* form (dodging 07.1's oracle/external-validity reasons); follows 07.4's documented-null follow-up (targeted surfacing, not a full dump). Graded **mechanistically + directionally, not significance-gated** (CK25 is underpowered at n=49; QALD's floor is an orthogonal entity-grounding bottleneck): adopt on shape-coverage of held-out failures + CK25 directional lift with zero regressions + QALD non-regression, all **overlap-audited** (bank↔test query-shape overlap reported; lifts read as an upper bound until overlap confirmed low). **CLOSED via the ADOPT path (07.5-05, credentialed 3-run live sweep, gpt-4o-mini, corpus_sha `814d227`) — with REQ-3 faithfulness recorded as DOCUMENTED-PARTIAL, not a clean pass:** the mechanically-generated (never hand-curated) CK25 bank reproduced a statistically significant, execution-graded directional lift on all 3 independent same-session runs (generated-fewshot vs a freshly-run zero arm, paired McNemar): run 1 14/49 vs 5/49 (b=10/c=1/p=0.0117), run 2 13/49 vs 5/49 (b=9/c=1/p=0.0215), run 3 14/49 vs 6/49 (b=8/c=0/p=0.0078) — bootstrap deltas +0.163 to +0.184, all CIs excluding 0, essentially unchanged after excluding the one shape+entity-overlap-audited near-duplicate case (`ck25-4`, D-05). `c==0` only on run 3; runs 1–2 each carry exactly one regression, both the SAME known-flaky case (`ck25-50`, a typo'd compound top-N+count question) — the SPEC §11.2 bar is met via its documented "or at minimum `b > c` on every run" alternative clause (10>1, 9>1, 8>0). Shape-coverage is 100% under the coarse shape-type reading (the literal canonical-skeleton reading is degenerately 0% by design — that disjointness IS the overlap-exclusion axis, not a coverage gap; both readings and the SPEC's underspecification are recorded). QALD-9-plus non-regression is trivially satisfied — the generated QALD bank is honestly empty (D-04), so the generated-fewshot arm mechanically IS the zero-shot arm (0/514, 0/514, 2/514 across the 3 runs vs. a prior committed 1/514 reference — noise, not a signal, at QALD's own achieved MDE). **REQ-3's secondary ≥95% faithfulness-judge bar is NOT met and is recorded honestly as DOCUMENTED-PARTIAL, never reframed as a pass:** the specified gpt-4o-mini judge (raw match 76–84% across sampled configs) produced ≥2 provable false-negatives per run on correct implicit-domain grouped-aggregation queries even when given domain/range context — an unreliable certification instrument, not evidence of a faithfulness defect; manual domain/range adjudication of the same pairs estimated true faithfulness at ~88–96%. This gap is non-blocking for the adopted lever because paraphrases (the artifact the judge evaluates) are never consumed by `FewShotIndex` retrieval or generation — only `question`+`query` are. The 3-run sweep predates the two_hop NL-question wording fix (commit `20f40a0`, changes only 8 questions' TEXT, every QUERY byte-identical) — a re-sweep was judged unnecessary since the measured lift is query-pattern-driven, and the bank committed alongside this closure already carries the fixed wording. — acceptance: mechanistic + directional grading (not significance-gated, CK25 underpowered at n=49) — shape-coverage ≥70% of held-out failures [MET, 100%, coarse reading]; CK25 directional lift with zero regressions, all 3 runs, overlap-audited [MET via the documented `b>c`-on-every-run alternative clause]; QALD non-regression [MET, trivially]; REQ-3 secondary faithfulness ≥95% judge-match [NOT MET — documented-partial, ~88–96% by manual adjudication] — evidence: `tests/nl2sparql/eval/baseline.json:phase07_5_generated_fewshot_sweep`, `tests/nl2sparql/eval/README.md` §11, `tests/nl2sparql/eval/vendored/ck25/generated_fewshot_bank.yml`
...
| REQ-thirdparty-tool-compat | Phase 4 | Complete |
| REQ-ontoextract-integration | Phase 4 | Complete |
| REQ-performance-slos | Phase 4 | Complete |
...
| NL-ACC-03 | Phase 07.6 | Complete (documented-null) |
