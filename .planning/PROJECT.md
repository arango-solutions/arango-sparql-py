...
- ✓ NL→SPARQL relationship-path grounding (engine **seam-8** `ClassPathIndex` — subclass-aware, inverse-edged, bounded (depth ≤3, ≤5-path) shortest-path over a mechanically-built TBox class-connectivity graph; stdlib-only / `pyoxigraph`-free; mechanical TBox-only so CDF-transferable and Cypher inherits it; both SPARQL adapters + `NlPipeline` threaded; engine promoted to both remotes + pin bumped → `1ec6789`): built, unit-tested, promoted, and honestly measured. **NL-ACC-03 closed via the DOCUMENTED-NULL path** — offline path-recall GREEN (14/16) but no CK25 accuracy lift composed additively on grounding+few-shot (`g+f+path` 15/49 vs `g+f` 16/49, b=0/c=1/p=1.0/Δ=−0.0204); mechanical recall ≠ live accuracy. `g+f` (16/49) remains the strongest composition; transpiler untouched, W3C ≥ 96.4% held — Phase 07.6
...
---
*Last updated: 2026-08-10 after Phase 07.6 completion (relationship-path grounding, seam 8). NL-ACC-03 closed via the documented-null path on a valid execution-graded experiment: the seam-8 path-hint shows no CK25 lift composed additively on grounding+few-shot (`g+f+path` 15/49 vs `g+f` 16/49, b=0/c=1/p=1.0), despite offline path-recall holding GREEN (14/16) — mechanical recall ≠ live accuracy. Prior since last footer update: Phase 07.5 ADOPTED the query-first synthetic few-shot bank (NL-GEN-01, promoted to `arango_query_core.nl.synthbank`). Entity grounding (seam 6, Phase 07.3) remains the sole confirmed CK25 lever; seams 7 (predicate, 07.4) and 8 (path, 07.6) both close documented-null. `g+f` (grounding + few-shot) is the strongest composition after four grounding-lever phases (07.3–07.6). Transpiler untouched, W3C DAWG ≥ 96.4% held throughout.*
